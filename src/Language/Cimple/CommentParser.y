...
Paragraph :: { [NonTerm] }
Paragraph
:	Word(NonInt) Punctuation MaybeWords			{ Fix (DocSentence [$1] (unT $2)) : $3 }
...
|	'@retval' Atom x					{ Fix $ DocRetval (unT $2) $3 }
...
|	'@ref' Atom						{ Fix $ DocRef (unT $2) }
|	'@p' Atom						{ Fix $ DocP (unT $2) }
...
{
newtype Term = T { unT :: Lexeme Text }
    deriving (Show)
type NonTerm = Comment (Lexeme Text)
...
parseError (T n:_, [])      = failAt n "; expected end of comment"
parseError (T n:_, options) = failAt n $ "; expected one of " <> show options
}
